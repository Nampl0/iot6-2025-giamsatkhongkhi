...
class MQTTService {
  ...
  connect() {
    ...
    this.mqttClient.on("connect", () => {
      ...
      this.mqttClient.subscribe("device/control", () => {
        console.log("📡 Subscribed to device/control");
      });
      this.mqttClient.subscribe("led/history");
      this.mqttClient.subscribe("fan/history");
    });
  ...
  // Publish MQTT Message truyền
  publish(topic, message, options) {
    this.mqttClient.publish(topic, message);
  }

  // Subscribe to MQTT Message nhận
